...
module.exports = function convertCoords(gd, ax, newType, doExtra) {
    ...
    var toSignlog = (newType === 'signlog') && (ax.type === 'linear');
    var fromSignlog = (newType === 'linear') && (ax.type === 'signlog');

    if(!(toLog || fromLog)) return;
    if(!(toSignlog || fromSignlog)) return;
    ...
    function convert(attr) {
        ...
        if(toLog || toSignlog) newVal = toLogRange(currentVal, ax.range);
