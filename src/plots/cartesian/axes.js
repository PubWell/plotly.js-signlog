...
axes.minDtick = function(ax, newDiff, newFirst, allow) {
    ...
    if(['log', 'signlog', 'category', 'multicategory'].indexOf(ax.type) !== -1 || !allow) {
        ax._minDtick = 0;
...
axes.autoBin = function(data, ax, nbins, is2d, calendar, size) {
    ...
    var dummyAx;
    if(ax.type === 'log' || ax.type === 'signlog') {
        dummyAx = {
            type: 'linear',
...
axes.calcTicks = function calcTicks(ax, opts) {
    ...
    for(var major = 1; major >= (hasMinor ? 0 : 1); major--) {
        ...
        var isDSignlog = (type === 'signlog') && !(numDtick || mockAx.dtick.charAt(0) === 'L');
        ...
        if(mockAx.rangebreaks && mockAx._tick0Init !== mockAx.tick0) {
            ...
            if(!axrev) {
                x = axes.tickIncrement(ax, x, dtick, !axrev, calendar);
            }
        }
        ...
        for(;
            ...
            if(major) {
                ...
                if(isDSignlog) {
                    var m = Math.log(Math.abs(ax.l2c(x))) / Math.LN10;
                    if(Math.abs(Math.round(m) - m) > Math.pow(10, -6)) obj.simpleLabel = true;
                }
...
function arrayTicks(ax) {
    ...
    if(ax.type === 'signlog' && String(ax.dtick).charAt(0) !== 'L') {
        var rngMin = Math.log(Math.abs(ax.l2c(ax.range[0]))) / Math.LN10;
        var rngMax = Math.log(Math.abs(ax.l2c(ax.range[1]))) / Math.LN10;
        ax.dtick = 'L' + Math.pow(10, Math.floor(Math.min(rngMin, rngMax)) - 1);
    }
...
axes.tickIncrement = function(ax, x, dtick, axrev, calendar) {
    ...
    if(tType === 'D') {
        ...
        if(ax.type == 'signlog'){
          var signDtickTrans = (signal, val) =>{
            var mark = signal > 0 ? axrev : !axrev;
            var frac = Lib.roundUp(Lib.mod(val, 1), tickset, mark);
            return Math.floor(val) + Math.log(d3.round(Math.pow(10, frac), 1)) / Math.LN10;
          }

          return filterSignlog(x2, ax.signpower, signDtickTrans)
        }
...
function filterSignlog(r0, power, tickSignTrans){
    var real, res, x1, endR, mpow = Math.pow(10, power);
    if(r0 < -1){
      x1 = -r0 - power;
      endR = tickSignTrans(-1, x1);
      res = -endR - power;
      if(res < -1){
        return res
      }else{
    ...
    if(r0 >= -1 && r0 < 1){
      real = r0 * 10 / mpow;
      var signal = Math.sign(r0);
      x1 = Math.log(Math.abs(real)) / Math.LN10;
      endR = tickSignTrans(signal, x1);
      res = signal * Math.pow(10, endR) * mpow / 10;
      if(res < 1 && res >= -1){
        return res
      }else{
        r0 = 1;
...
axes.tickFirst = function(ax, opts) {
    ...
    if(isNumeric(dtick)) {
        ...
        if(ax.type == 'signlog'){
            var signVal, logVal, mark = axrev ? 1 : -1;
    
            signVal = ax.l2c(r0);
            logVal = Math.log(Math.abs(signVal)) / Math.LN10;
            sRound = Math.sign(signVal) * mark > 0 ? Math.floor : Math.ceil;
    
            var newVal = sRound((logVal - tick0) / dtick) * dtick + tick0;
            tmin = ax.c2l(Math.sign(signVal) * Math.pow(10, newVal));
        }
    ...
    if(tType === 'M') {
        ...
        var real = ax.l2c(r0);
        var newReal = sRound( (real - tick0) / dtNum ) * dtNum + tick0;
        return ax.c2l( newReal );
    } else if(tType === 'D') {
        var tickset = (dtick === 'D2') ? roundLog2 : roundLog1;
        ...
        if(ax.type == 'signlog'){
          ...
          return filterSignlog(r0, ax.signpower, signDtickTrans)
        }
...
axes.tickText = function(ax, x, hover, noSuffixPrefix) {
    ...
    else if(axType === 'log' || axType === 'signlog') formatLog(ax, out, hover, extraPrecision, hideexp);
...
function formatLog(ax, out, hover, extraPrecision, hideexp) {
    ...
    if(tickformat || (dtChar0 === 'L')) {
        out.text = numFormat(real, ax, hideexp, extraPrecision);
    } else if(isNumeric(dtick) || ((dtChar0 === 'D') && (Lib.mod(realLog + 0.01, 1) < 0.1))) {
        var p = Math.round(realLog);
        var absP = Math.abs(p);
        var exponentFormat = ax.exponentformat;
        if(exponentFormat === 'power' || (isSIFormat(exponentFormat) && beyondSI(p))) {
            if(p === 0) out.text = Math.sign(real) * 1;
            else if(p === 1) out.text = (real > 0 ? '' : MINUS_SIGN) + '10';
            else out.text = (real > 0 ? '' : MINUS_SIGN) + '10<sup>' + (p > 1 ? '' : MINUS_SIGN) + absP + '</sup>';

            out.fontSize *= 1.25;
        }else if((exponentFormat === 'e' || exponentFormat === 'E') && absP > 2) {
            out.text = (real > 0 ? '' : MINUS_SIGN) + '1' + exponentFormat + (p > 0 ? '+' : MINUS_SIGN) + absP;
        }else {
            out.text = numFormat(real, ax, '', 'fakehover');
            if(dtick === 'D1' && ax._id.charAt(0) === 'y') {
                out.dy -= out.fontSize / 6;
            }
        }
    } else if(dtChar0 === 'D') {
        out.text = String(Math.round(Math.pow(10, Lib.mod(realLog, 1))));
        out.fontSize *= 0.75;
    } else throw 'unrecognized dtick ' + String(dtick);
        ...
        if(firstChar === '0' || firstChar === '1') {
            if(ax._id.charAt(0) === 'y') {
                ...
                out.dx += (ax.range[1] > ax.range[0] ? 1 : -1) *
                    out.fontSize * (realLog < 0 ? 0.5 : 0.25);
            }
        }
    }
}
...
axes.drawLabels = function(gd, ax, opts) {
    ...
    function fixLabelOverlaps() {
        ...
        if(vals.length && axLetter === 'x' && !isNumeric(tickAngle) &&
            ((ax.type !== 'log' && ax.type !== 'signlog') || String(ax.dtick).charAt(0) !== 'D')
        ) {
            autoangle = 0;
...
function swapAxisGroup(gd, xIds, yIds) {
    ...
    var numericTypes = ['linear', 'log', 'signlog'];
