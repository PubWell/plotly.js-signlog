...
module.exports = function setConvert(ax, fullLayout) {
    ...
    function fromSignlog(val) {
        let mPow = Math.pow(10, ax.signpower);

        if (val > -1 && val < 1){
            return val * 10 / mPow;
        }
        else {
            return Math.sign(val) * Math.pow(10, Math.abs(val)) / mPow;
        }
    }
  
    function toSignlog(val, clip){
        var qPow = Math.pow(10, ax.signpower);
        val = qPow * val;
        ...
        if (val > -10 && val < 10) {
            ...
            return Math.sign(val) * Math.log(Math.abs(val)) / Math.log(10);
        }
    }
    ...
    ax.c2l = (ax.type === 'log') ? toLog : (ax.type === 'signlog') ? toSignlog : ensureNumber;
    ax.l2c = (ax.type === 'log') ? fromLog : (ax.type === 'signlog') ? fromSignlog : ensureNumber;

    ax.l2p = l2p;
    ax.p2l = p2l;

    ax.c2p = (ax.type === 'log') ? function(v, clip) { return l2p(toLog(v, clip)); } : (ax.type === 'signlog') ? function(v, clip) { return l2p(toSignlog(v, clip)); } : l2p;
    ax.p2c = (ax.type === 'log') ? function(px) { return fromLog(p2l(px)); } : (ax.type === 'signlog') ? function(px) { return fromSignlog(p2l(px)); } : p2l;
    ...
    if(['linear', '-'].indexOf(ax.type) !== -1) {
        ...
        // d and c are data vals, r and l are logged (but d and r need cleaning)
        ax.d2r = ax.d2l = function(v, clip) { return toSignlog(cleanNumber(v), clip); };
        ax.r2d = ax.r2c = function(v) { return fromSignlog(cleanNumber(v)); };
        ...
        ax.d2p = function(v, clip) { return ax.l2p(ax.d2r(v, clip)); };
        ax.p2d = function(px) { return fromSignlog(p2l(px)); };
    ...
    ax.makeCalcdata = function(trace, axLetter, opts) {
        ...
        if(axLetter in trace) {
            ...
            if(Lib.isTypedArray(arrayIn) && (axType === 'linear' || axType === 'log' || axType === 'signlog')) {
