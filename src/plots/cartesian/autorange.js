...
function doAutoRange(gd, ax, presetRange) {
    ...
    if(ax.autorange) {
        ...
        edits[ax._attr + '.extremes'] = ax.extremes;
        edits[ax._attr + '.signpower'] = ax.signpower;
        edits[ax._attr + '.ptvmin'] = ax.ptvmin;
        edits[ax._attr + '.ntvmax'] = ax.ntvmax;
        Registry.call('_storeDirectGUIEdit', gd.layout, gd._fullLayout._preGUI, edits);

        axIn.range = ax.range.slice();
        axIn.autorange = ax.autorange;
        axIn.extremes = ax.extremes;
        axIn.signpower = ax.signpower;
        axIn.ptvmin = ax.ptvmin;
        axIn.ntvmax = ax.ntvmax;
    }
...
function findExtremes(ax, data, opts) {
    ...
    var isSignlog = ax.type === 'signlog';
    var hasArrayOption = false;
    var vpadLinearized = opts.vpadLinearized || false;
    var i, v, di, dmin, dmax, ppadiplus, ppadiminus, vmin, vmax, ntvList, ptvList, nmax, pmin;
    ...
    function calcNtvPtv(ntvMax, ptvMin) {
        ptvMin = ptvMin > 0 ? ptvMin : 0;
        ntvMax = ntvMax < 0 ? ntvMax : 0;
        
        ntvMax = Math.min(ntvMax, 0);
        ptvMin = Math.max(ptvMin, 0);
  
        let min = 0;
        if (ntvMax !== 0 && ptvMin !== 0) {
          min = Math.min(-ntvMax, ptvMin);
        }
        else if (ntvMax === 0) {
          min = ptvMin;
        }
        ...
        if (min > 0 && min < 10) {
          ax.signpower = Math.abs(Math.log(min) / Math.log(10));
        }
    }
    ...
    if(!hasArrayOption) {
        ...
        var minLimit = isLog ? 0 : -FP_SAFE;
        
        for(i = 0; i < len; i++) {
          v = data[i];
          if(v < 0) ntvList.push(v);
          if(v > 0) ptvList.push(v);
          if(v < vmin && v > minLimit) vmin = v;
          if(v > vmax && v < FP_SAFE) vmax = v;
        }
        if(ntvList.length){
          ntvList.sort((a,b) => Number(a) - Number(b));
          nmax = ntvList[ntvList.length - 1];
        }
        if(ptvList.length){
          ptvList.sort((a,b) => Number(a) - Number(b));
          pmin = ptvList[0];
        }
        if(ax.extremes && ax.extremes.length){
          if(vmin < ax.extremes[0]){
            ax.extremes[0] = vmin;
          }
          if(vmax > ax.extremes[1]){
            ax.extremes[1] = vmax;
          }
          ...
          ax.extremes = [vmin, vmax];
          ax.ptvmin = pmin;
          ax.ntvmax = nmax;
        }
        calcNtvPtv(ax.ntvmax, ax.ptvmin);
